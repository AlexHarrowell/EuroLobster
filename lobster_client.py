...
argparser.add_argument('-o', '--output', type=lambda t: unicode(t, e), default=None)
		...
		if self.user_input['output'] == u'csv':
			if self.user_input['filename']:
				with open(self.user_input['filename'], 'w') as f:
					c = csv.DictWriter(f, fieldnames=output.keys())
					c.writeheader()
					for k in ks:
						row = {m: m.get(k, 0) for m in output}
						c.writerow(row)
			else:
				with StringIO.StringIO() as s
					c = csv.DictWriter(s, fieldnames=output.keys())
					c.writeheader()
					for k in ks:
						row = {m: m.get(k, 0) for m in output}
						c.writerow(row)
